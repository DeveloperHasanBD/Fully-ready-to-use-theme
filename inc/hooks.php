...
if ( ! function_exists( 'understrap_add_site_info' ) ) {
	...
	function understrap_add_site_info() {
		...
		$site_info = sprintf(
			...
			esc_url( __( 'https://wordpress.org/', 'urban' ) ),
			sprintf(
				/* translators: WordPress */
				esc_html__( 'Proudly powered by %s', 'urban' ),
				'WordPress'
			),
			sprintf( // WPCS: XSS ok.
				/* translators: 1: Theme name, 2: Theme author */
				esc_html__( 'Theme: %1$s by %2$s.', 'urban' ),
				$the_theme->get( 'Name' ),
				'<a href="' . esc_url( __( 'https://urban.com', 'urban' ) ) . '">urban.com</a>'
			),
			sprintf( // WPCS: XSS ok.
				/* translators: Theme version */
				esc_html__( 'Version: %1$s', 'urban' ),
